...
const wrapper: Wrapper<Vue> = shallowMount(Compare, { propsData: {}, vuetify });
  ...
  describe('compare charts', () => {
    ...
    it('compliance chart gets correct data with 2 files', async () => {
      ...
      expect(
        new Set(
          (
            wrapper.vm as Vue & { compliance_series: SeriesItem[] }
          ).compliance_series[0].data,
        ),
      ).toEqual(
        new Set([
          calculateCompliance({ fromFile: [FilteredDataModule.selected_file_ids[0]] }),
          calculateCompliance({ fromFile: [FilteredDataModule.selected_file_ids[1]] }),
        ]),
      );
    });
