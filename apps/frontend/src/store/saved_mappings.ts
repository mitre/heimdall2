...
const mutations = {
  ADD_MAPPING(
    state: MappingsState,
    {id, mapping}: {id: string; mapping: DictionaryObject}
  ) {
  ...
  UPDATE_MAPPING(
    state: MappingsState,
    {id, updatedMapping}: {id: string; updatedMapping: DictionaryObject}
  ) {
  ...
  ADD_DESCRIPTION(
    state: MappingsState,
    {
      id,
      descriptions
    }: {id: string; descriptions: {[mappingName: string]: string}}
  ) {
  ...
  UPDATE_DESCRIPTION(
    state: MappingsState,
    {
      id,
      updatedDescriptions
    }: {id: string; updatedDescriptions: {[mappingName: string]: string}}
  ) {
...
const actions = {
  addMapping(
    ...
    const existingRightParts = Object.keys(state.mappings).map(
      extractRightPart
    );
    if (existingRightParts.includes(newRightPart)) {
      console.error(
        `Mapping with the right part "${newRightPart}" already exists.`
      );
  ...
  addDescription(
    ...
    const newRightPart = extractRightPart(id);
    // Check if any existing ID has the same right part
    const existingRightParts = Object.keys(state.descriptions).map(
      extractRightPart
    );
    if (existingRightParts.includes(newRightPart)) {
      console.error(
        `Description with the right part "${newRightPart}" already exists.`
      );
