<template>
  <v-row dense>
    ...
    <v-col
      v-if="result['backtrace'] !== undefined"
      cols="12"
      class="pa-2"
    >
      <v-expansion-panels>
...
export default class ControlRowCol extends mixins(HtmlSanitizeMixin) {
  @Prop({ required: true, type: Object }) readonly result!: HDFControlSegment;
  @Prop({ required: true, type: String }) readonly statusCode!: string;
