<template>
  <v-container fluid class="font-weight-bold">
    <div
      ...
      >
          <!-- Override Validation Alert Row -->
          <v-row v-if=overideValidationError() v-resize="onResize">
            <v-col >
              <v-alert border="top" color="red" dark>
                Evaluation contains results that aren't assessed or approved
          ...
          <v-row v-resize="onResize">
            <v-row>
              ...
              <v-spacer />
              <v-col v-if=isDeveloperOrCertifier() cols="3" md="auto" class="text-right pl-6 pb-0">
                <v-switch
                  v-model="displayNewControls"
                  label="Show Only New Controls"
                />
              </v-col>
              <v-col v-if=isCyber() cols="3" md="auto" class="text-right pl-6 pb-0">
                <v-switch
                  v-model="displayPending"
                  label="CAB View"
                />
              </v-col>
        ...
        <ResponsiveRowSwitch :assessment_type=assessment_type()>
          <template #status>
            <ColumnHeader
              text="Status"
              :sort="sortStatus"
          ...
          <template v-if=isGenResult #compCWE>
            <ColumnHeader
              text="CWE"
              sort="disabled"
            />
          ...
          <template v-if=isStigResult #compVulnID>
            <ColumnHeader
              text="Vuln ID"
              :sort="sortVulnID"
              @input="set_sort('vulnID', $event)"
            />
          </template>
  ...
  export default class ControlTable extends Vue {
    ...
    @Prop({type: Boolean, default: false }) readonly isGenResult!: boolean;
    @Prop({type: Boolean, default: false }) readonly isVulnResult!: boolean;
    @Prop({type: Boolean, default: false }) readonly isStigResult!: boolean;
    ...
    sortVulnID: Sort = 'none';

    // Used for viewed/unviewed controls.
    viewedControlIds: string[] = [];
    displayUnviewedControls = true;

    isDeveloperOrCertifier(): boolean{
        if (ProductModuleState.viewMode == ViewModeType.Developer || ProductModuleState.viewMode == ViewModeType.Cyber){
          return true;
        }
        return false;
    }
    ...
    isCyber(): boolean{
      return ProductModuleState.viewMode === ViewModeType.Cyber;
    }
    ...
    assessment_type(): String {
      if (this.isVulnResult) {
        return 'vulnerability';
      } else if (this.isGenResult) {
        return 'general';
      } else if (this.isStigResult) {
        return 'stig';
      } else {
        return 'default';
      }
  ...
  get numOfViewed() {
    ...
    overideValidationError(): boolean{
      if (ProductModuleState.overrideValidation){
        for (let i of this.items) {
          //Check for failing and no overide
          if (i.control.hdf.status === 'Failed' && !i.control.hdf.hasControlStatusOverride) {
            return true;
          }
          //Check for overide status, and see if it's approved
          if (i.control.hdf.hasControlStatusOverride) {
            if (!i.control.hdf.isOverideStatusApproved) {
              return true;
            }
          }
        }
    ...
    get displayPending(){
       return _.has(SearchModule.statusFilter,'Pending');
    }

    set displayPending(value: boolean){
      if (value){
        SearchModule.addStatusFilter('Pending');
      }else{
    ...
    get items(): ListElt[] {
      ...
      if (this.sortId === 'ascending' || this.sortId === 'descending') {
        ...
        this.sortVulnID === 'ascending' ||
        this.sortVulnID === 'descending'
      ) {
        cmp = (a: ListElt, b: ListElt) =>
          a.control.data.tags.gid.localeCompare(b.control.data.tags.gid);
        if (this.sortVulnID === 'ascending') {
          factor = -1;
        }
      } else {
        sort = false;
      ...
      // Displays only new controls.
      if (ProductModuleState.displayNewControls) {
        items = items.filter(
          (val) => this.hasNewResults(_.get(val,'control.data'))
        );
      }
    ...
    // hasNewResults filters controls
    // Identifies controls with override information for exclusion.
    // If this toggle is set, all results for an rules that have override data will be hidden.
    hasNewResults(obj: object) {
      let results = _.get(obj, 'results');
      return !_.isEmpty(_.find(results, function(o: any) { return o.override == undefined}));
    }
  }
  </script>
