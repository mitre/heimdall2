<template>
  ...
  >
    ...
    >
      ...
      >
        <v-icon title="Save entry to the database">
...
export default class SidebarFileList extends mixins(ServerMixin, RouteMixin) {
  ...
  get icon(): string {
    return Object.hasOwn(this.file, 'profile') ? 'mdi-note' : 'mdi-google-analytics';
  }
  ...
  save_to_database(file: EvaluationFile | ProfileFile) {
    ...
    formData.append(
      'data',
      new Blob([JSON.stringify(this.file_data(file))], { type: 'text/plain' }),
    );
    axios
      .post<IEvaluation>('/evaluations', formData)
      .then((response) => {
        SnackbarModule.notify('File saved successfully');
        file.database_id = Number.parseInt(response.data.id);
