...
<script lang="ts">
import type { IApiKey, IGroup } from '@heimdall/common/interfaces';
...
export default class GroupAPIKeysModal extends Vue {
  ...
  deleteAPIKey(item: IApiKey) {
    this.loading = true;
    axios
      .delete<IApiKey>(`/apikeys/${item.id}`, { data: { currentPassword: this.password } })
      .then(({ data }) => {
  ...
  updateAPIKeys() {
    this.loading = true;
    axios
      .get<IApiKey[]>('/apikeys', { params: { groupId: this.group.id } })
      .then(({ data }) => {
        this.apiKeys = data;
        this.loading = false;
      })
