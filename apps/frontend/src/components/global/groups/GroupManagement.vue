<template>
  <v-card>
    <v-card-title class="pb-0">
      <GroupModal id="groupModal" :create="true">
        <template #clickable="{ on, attrs }">
          <v-btn color="primary" data-cy="createNewGroupBtn" class="mb-2" v-bind="attrs" v-on="on">
            Create New Group
          </v-btn>
        </template>
      </GroupModal>
    ...
    <v-data-table :headers="groupsHeaders" :items="groupData" class="elevation-0" dense :loading="loading"
      ...
      <template #top>
        <v-container>
          <v-text-field v-model="search" append-icon="mdi-magnify" label="Search" single-line dense hide-details />
        </v-container>
      </template>
    ...
    <ActionDialog v-model="dialogDelete" type="group" @cancel="closeActionDialog" @confirm="deleteGroupConfirm" />
    <GroupUsers v-model="selectedGroupUsers" :dialog-display-users="dialogDisplayUsers"
