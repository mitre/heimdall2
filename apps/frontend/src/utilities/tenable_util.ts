...
export class TenableUtil {
  ...
  isServer: boolean = ServerModule.serverMode ? (ServerModule.forceTenableFrontend ? false : true) : false;
    ...
    if (error.response) {
      ...
      if (error.response.data.error_code == 74 || error.status === 403) {
        ...
        if (error.status == 400) {
          ...
          rejectMsg = this.isServer && !TENABLE_HOST_URL ? TENABLE_CSP_NOT_SET : `Network Error -> ${DEFAULT_REJECT_MSG}`;
  ...
  async getVulnerabilities(scanId: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.axios_instance
          .post(url, {}, { headers: { 'Content-Type': 'application/zip' }, responseType: 'arraybuffer' })
          .then(async (response) => {
