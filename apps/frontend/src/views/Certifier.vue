...
import ControlTableSelector from '../components/cards/controltable/ControlTableSelector.vue';
...
export default class Certifier extends mixins(RouteMixin, ServerMixin) {
  /**
   * The current state of the modeled treemap
   * if a "deep" selection is not null, then its parent should also be not-null
   */
  ...
  mounted(){
    ProductModuleState.UpdateOverrideValidation(true);
  }
  ...
  get firstEval(): SourcedContextualizedEvaluation | undefined{
        return _.first(Array.from(FilteredDataModule.evaluations(this.file_filter)));
  ...
  get is_result_view(): boolean {
    return this.current_route === 'certifier';
  }
