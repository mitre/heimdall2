...
function createAttribute(name: string, value: { path: string }, required = false, sensitive = false, type = null) {
  return { name: name, options: { value: value, required: required, sensitive: sensitive, type: type } }
}

const mappings: MappedTransform<ExecJSON, LookupPath> = {
  platform: {
    name: 'Heimdall Tools',
    target_id: `${{ path: 'last_run.ruleset_name' }} ruleset: ${{ path: 'provider_name' }}:${{ path: 'account_id' }}`,
    release: HeimdallToolsVersion,
  },
  profiles: [{
    name: 'Scout Suite Multi-Cloud Security Auditing Tool',
    version: { path: 'last_run.version' },
    title: `Scout Suite Report using ${{ path: 'last_run.version' }} ruleset on ${{ path: 'provider_name' }} with account ${{ path: 'account_id' }}`,
    attributes: [
      { account_id: { path: 'account_id' } },
      createAttribute('environment', { path: 'environment' }),
      createAttribute('ruleset', { path: 'ruleset_name' }),
      createAttribute('run_parameters_excluded_regions', { path: 'last_run.run_parameters.excluded_region' }),
      createAttribute('run_parameters_regions', { path: 'last_run.run_parameters.regions' }),
      createAttribute('run_parameters_services', { path: 'last_run.run_parameters.services' }),
      createAttribute('run_parameters_skipped_services', { path: 'last_run.run_parameters.skipped_services' }),
