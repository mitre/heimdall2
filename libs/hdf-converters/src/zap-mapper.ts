...
function filterSite<T>(input: Array<T>, name?: string) {
  ...
  if (name) {
    return input.find((element) => (_.get(element, '@name') as unknown as string) === name);
  }
  // Otherwise choose the site with the most alerts
  else {
    return input.reduce((a, b) =>
      (_.get(a, 'alerts') as Record<string, unknown>[]).length > (_.get(b, 'alerts') as Record<string, unknown>[]).length ? a : b
    );
  }
}
