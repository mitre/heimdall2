...
function replaceTypesSlashes<T>(type: T): string | T {
  if (!_.isString(type)) {
    return type;
  }
...
function objectifyTypesArray(
  typesArray: Record<string, unknown> | string[],
...
function mapping(
  ...
  const profileNames = Object.keys(executionTypes || {}).filter(
    type =>
      !['Execution', 'File', 'HDF2ASFF-converter', 'MITRE'].includes(type),
  );
  return {
    platform: {
              ...
              (finding: Record<string, unknown>) => {
                ...
                return {
                  ...
                  descriptions: _.map(
                    ...
                    ([key, value]) => ({ data: value, label: key }),
                  ),
                  ...
                  tags: {
                    ..._.omit(
                      _.get(findingTypes, 'Tags') as null | object | undefined,
                      ['nist'],
                    ),
                    nist: ((): string[] => {
