...
describe('CAAT Results Reverse Mapper', () => {
  it('Successfully converts two RHEL HDF and a RHEL triple overlay HDF into CAAT', () => {
    ...
    const expected = XLSX.readFile('sample_jsons/caat_reverse_mapper/caat.xlsx', { type: 'file' });

    // convert workbooks to json to compare just the content instead of random bits of xlsx structure
    expect(converted.SheetNames.map(name => XLSX.utils.sheet_to_json(expected.Sheets[name]))).toEqual(expected.SheetNames.map(name => XLSX.utils.sheet_to_json(expected.Sheets[name])));
