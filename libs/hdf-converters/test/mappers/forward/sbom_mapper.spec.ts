...
describe('sbom_mapper_vex', () => {
  it('Successfully converts SBOM data', () => {
    ...
    // fs.writeFileSync(
    //   'sample_jsons/sbom_mapper/sbom-vex-hdf.json',
    //   JSON.stringify(mapper.toHdf(), null, 2)
    // );
    ...
    expect(omitVersions(mapper.toHdf())).toEqual(
      omitVersions(
        JSON.parse(
          fs.readFileSync('sample_jsons/sbom_mapper/sbom-vex-hdf.json', {
            encoding: 'utf-8'
          })
        )
      )
  ...
  it('Successfully converts withraw flagged SBOM data', () => {
    ...
    // fs.writeFileSync(
    //   'sample_jsons/sbom_mapper/sbom-vex-hdf-withraw.json',
    //   JSON.stringify(mapper.toHdf(), null, 2)
    // );
