...
const attestationMessageArray: Attestation[] = [
  {
    control_id: 'V-72087',
    explanation:
      'This is a passing attestation that is not expired',
    frequency: '1d',
...
const aPass_cPass_unexp: Attestation[] = [
  {
    ...
    explanation:
      'This is a test explanation for 230328',
    frequency: 'monthly',
    status: ExecJSON.ControlResultStatus.Passed,
    updated: '3000-01-01',
    updated_by: 'Attestation Tester'
...
const aPass_cFail_unexp: Attestation[] = [
  {
    ...
    explanation:
      'This is a test explanation for 230222',
    frequency: '1d',
    status: ExecJSON.ControlResultStatus.Passed,
    updated: '3000-01-01',
    updated_by: 'Attestation Tester'
...
describe('convertAttestationToSegment', () => {
  ...
  it('Should convert a passing and unexpired attestation message to an HDF file segment', () => {
    const segment_unexpired_pass = convertAttestationToSegment(attestationMessageArray[0]);
    expect(segment_unexpired_pass.status).toEqual(ExecJSON.ControlResultStatus.Passed);
    expect(segment_unexpired_pass.message).toEqual(createAttestationMessage(attestationMessageArray[0], false));
  });

  it('Should convert a passing and expired attestation message to an HDF file segment', () => {
    const segment_expired_pass = convertAttestationToSegment(attestationMessageArray[1]);
    expect(segment_expired_pass.status).toEqual(ExecJSON.ControlResultStatus.Skipped);
    expect(segment_expired_pass.message).toEqual(createAttestationMessage(attestationMessageArray[1], true));
  });

  it('Should convert a failing and unexpired attestation message to an HDF file segment', () => {
    const segment_unexpired_fail = convertAttestationToSegment(attestationMessageArray[2]);
    expect(segment_unexpired_fail.status).toEqual(ExecJSON.ControlResultStatus.Failed);
    expect(segment_unexpired_fail.message).toEqual(createAttestationMessage(attestationMessageArray[2], false));
  });

  it('Should convert a failing and expired attestation message to an HDF file segment', () => {
    const segment_expired_fail = convertAttestationToSegment(attestationMessageArray[3]);
    expect(segment_expired_fail.status).toEqual(ExecJSON.ControlResultStatus.Skipped);
    expect(segment_expired_fail.message).toEqual(createAttestationMessage(attestationMessageArray[3], true));
  });
});

describe('addAttestationToHDF', () => {
  let inputData: ExecJSON.Execution;
  let consoleOriginal = console.error;
  ...
  it('Should add an unexpired attestation to a skipped control', () => {
    ...
    expect(output.profiles[0].controls[2].results[1].status).toEqual(ExecJSON.ControlResultStatus.Passed);
    // Check that the attestation data added to the control is the equal to the attestation passed into the function
    expect(output.profiles[0].controls[2].attestation_data).toEqual(aPass_cSkip_unexp[0]);
  ...
  it('Should add an expired attestation to a skipped control', () => {
    ...
    expect(output.profiles[0].controls[2].results.length).toEqual(2);
    // Check that the status of the new result is passing
    expect(output.profiles[0].controls[2].results[1].status).toEqual(ExecJSON.ControlResultStatus.Skipped);
    // Check that the attestation data added to the control is the equal to the attestation passed into the function
    expect(output.profiles[0].controls[2].attestation_data).toEqual(aPass_cSkip_exp[0]);
  ...
  it('Should not add an attestation to a passing control', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('Invalid control selected: Control must have \"skipped\" status to be attested');
  });
  ...
  // Check when adding multiple attestations, an expired then an unexpired one
  it('Should add and overwrite an attestation to a control that has already an expired attestation', () => {
    ...
    const output= addAttestationToHDF(addAttestationToHDF(inputData, aPass_cSkip_exp), aPass_cSkip_unexp);
    ...
    // Check that the status of the new result is passing
    expect(output.profiles[0].controls[2].results[2].status).toEqual(ExecJSON.ControlResultStatus.Passed);
    // Check that the attestation data added to the control is the equal to the attestation passed into the function
...
// You can also test if they add successfully, but I don't think that matters 
