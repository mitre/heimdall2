...
installed. See [`packaging/rpm/setup-rpm-build-env.sh`](setup-rpm-build-env.sh)
...
1. **Configuration** — generates `/etc/heimdall-server/backend.env` with
   database credentials, JWT secrets, and API key secrets. Missing values
   are auto-generated securely.
...
4. **Security policies** — registers the Heimdall port with SELinux, adds
   bundled binaries to fapolicyd trust, enables the firewalld service.
   Each subsystem is skipped if its tools are not installed.
5. **Service start** — enables and starts `heimdall-server.service`.
...
Access Heimdall at `http://<your-host>:3000`.
...
For the complete list of all 75 environment variables, see
[`ENVIRONMENT_VARIABLES.md`](../../ENVIRONMENT_VARIABLES.md) in the
project root, or run `heimdall-cli config list`.

### Key Settings

| Variable | Default | Description |
|---|---|---|
| `HEIMDALL_BACKEND_PORT` | `3000` | HTTP listen port for the Heimdall backend |
...
If `firewalld` is running, open the port:
...
For production deployments behind nginx with TLS:
...
server {
    ...
    location / {
        proxy_pass http://127.0.0.1:3000;  # Must match HEIMDALL_BACKEND_PORT in backend.env
...
Set `NGINX_HOST` and `EXTERNAL_URL` in `backend.env` to match your public
hostname. `EXTERNAL_URL` is required for OAuth/OIDC callback URLs to work.
...
If you change `HEIMDALL_BACKEND_PORT` in `backend.env` to something other than 3000, register
...
heimdall-cli config get HEIMDALL_BACKEND_PORT
sudo heimdall-cli config set HEIMDALL_BACKEND_PORT 8443
...
- Port already in use → change `HEIMDALL_BACKEND_PORT` in `backend.env`
