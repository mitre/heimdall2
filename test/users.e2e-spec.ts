...
describe('/users', () => {
  ...
  describe('POST', () => {
    ...
    it('should return 400 status if email is not provided', async () => {
      return await request(app.getHttpServer()).post('/users').set('Content-Type', 'application/json').send(CREATE_USER_DTO_TEST_OBJ_WITH_MISSING_EMAIL_FIELD)
    ...
    it('should return 400 status if password does not meet complexity requirements', async () => {
      return await request(app.getHttpServer()).post('/users').set('Content-Type', 'application/json').send(CREATE_USER_DTO_TEST_OBJ_WITH_INVALID_PASSWORD)
        .expect(HttpStatus.BAD_REQUEST).then(response => {
          expect(response.body.message).toEqual('Password does not meet complexity requirements. Passwords are a minimum of 15 characters in length. Passwords ' +
  ...
  describe(':id', () => {
    ...
    describe('GET', async () => {
      ...
      it('should return 400 status if given invalid token', async () => {
        const invalidID = -1;
        return await request(app.getHttpServer()).get('/users/' + invalidID).set('Authorization', 'bearer ' + 'badtoken').expect(HttpStatus.UNAUTHORIZED).then(response => {
    ...
    describe('POST', async () => {
      ...
      it('should return 400 status when password does not meet complexity requirements', async () => {
        return await request(app.getHttpServer()).put('/users/' + id).set('Authorization', 'bearer ' + jwtToken).send(UPDATE_USER_DTO_TEST_WITH_NOT_COMPLEX_PASSWORD).expect(HttpStatus.BAD_REQUEST).then(response => {
          expect(response.body.message).toEqual('Password does not meet complexity requirements. Passwords are a minimum of 15 characters in length. Passwords ' +
            'must contain at least one special character, number, upper-case letter, and lower-case letter. Passwords cannot contain more than three consecutive repeating ' +
            'characters. Passwords cannot contain more than four repeating characters from the same character class.');
          expect(response.body.error).toEqual('Bad Request');
      ...
      it('should return 400 status when password and passwordConfirmation dont match', async () => {
        return await request(app.getHttpServer()).put('/users/' + id).set('Authorization', 'bearer ' + jwtToken).send(UPDATE_USER_DTO_TEST_OBJ_WITH_MISSMATCHING_PASSWORDS).expect(HttpStatus.BAD_REQUEST).then(response => {
          expect(response.body.message).toEqual('Passwords do not match');
          expect(response.body.error).toEqual('Bad Request');
        });
      });
    ...
    describe('DELETE', async () => {
      ...
      it('should return 200 status after user is deleted by admin', async () => {
        ...
        await request(app.getHttpServer()).delete('/users/' + adminID).set('Authorization', 'bearer ' + adminJWTToken).send(DELETE_USER_DTO_TEST_OBJ).expect(HttpStatus.OK).then(response => {
          const createdAt = response.body.createdAt.valueOf();
          const updatedAt = response.body.updatedAt.valueOf();
          // User should have been created within the last minuted
          const createdWithinOneMinute = (new Date().getTime() - new Date(createdAt).getTime());
          ...
          expect(response.body.loginCount).toEqual('0');
          expect(response.body.organization).toEqual(CREATE_ADMIN_DTO.organization);
          expect(response.body.role).toEqual(CREATE_ADMIN_DTO.role);
          expect(response.body.title).toEqual(CREATE_ADMIN_DTO.title);
          expect(updatedWithinOneMinute).toBeLessThanOrEqual(MINUTE_IN_MILLISECONDS);
        });

        return await request(app.getHttpServer()).get('/users/' + adminID).set('Authorization', 'bearer ' + adminJWTToken).expect(HttpStatus.NOT_FOUND).then(response => {
