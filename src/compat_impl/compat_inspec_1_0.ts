...
abstract class HDFControl_1_0 implements HDFControl {
    ...
    constructor(forControl: ResultControl_1_0 | ProfileControl_1_0, is_profile: boolean, waived: boolean) {
        this.wraps = forControl;
        this.waived = waived;
        this.is_profile = is_profile;

        this.raw_nist_tags = HDFControl_1_0.compute_raw_nist_tags(this.wraps);
        let tmp = HDFControl_1_0.compute_proper_nist_tags(this.raw_nist_tags);
    ...
    private static compute_raw_nist_tags(raw: ResultControl_1_0 | ProfileControl_1_0): string[] {
        let fetched: string[] | undefined | null = raw.tags["nist"];
        if (!fetched) {
            return ["UM-1"];
        } else {
            return fetched;
    ...
    private static compute_proper_nist_tags(raw: string[]): [NistControl[], NistRevision | null] {
        // Initialize
        ...
        // Sort the tags
        parsed_nist_tags = parsed_nist_tags.sort((a, b) =>
            a.localCompare(b)
        );
    ...
    private static compute_severity(raw: ResultControl_1_0 | ProfileControl_1_0): Severity {
        if (raw.impact < 0.1) {
            return "none";
...
export class ExecControl extends HDFControl_1_0 implements HDFControl {
    ...
    private static compute_message(control: ResultControl_1_0): string {
        if (control.impact != 0) {
            ...
            return control.results
                .map(ExecControl.to_message_line)
                .join("");
        } else {
    ...
    private static compute_segments(control: ResultControl_1_0): HDFControlSegment[] {
